...
`drift-ui-canvas-trail` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for canvas scenarios with framed sample traffic, bounds and ordering tests, and no credentials or hosted services.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 5.
